...
public final class Validator {
   ...
   /**
    * Valida el identificador del transporte.
    *
    * @param identificador patente o código
    * @return true si es válido
    */
   public static boolean validarIdentificador(String identificador) {

       return noEsVacio(identificador)
               && longitudMinima(identificador, 4);

   }
   public static boolean validarEstado(String estado) {

       return noEsVacio(estado)
               && estado.trim().matches(
                       "(?i)Disponible|En servicio|Mantención"
               );
   ...
   public static boolean validarMarca(String marca) {
       ...
       return noEsVacio(marca)
               && longitudMinima(marca, 2);

   }
   public static boolean validarEslora(double eslora) {

       return eslora > 0;

   }
   /**
    * Valida el modelo de un vehículo.
    *
    * Debe contener al menos 2 caracteres y puede incluir
   ...
   public static boolean validarModelo(String modelo) {
       ...
       return noEsVacio(modelo)
               && longitudMinima(modelo, 2)
               && modelo.matches("[A-Za-z0-9áéíóúÁÉÍÓÚñÑ\\- ]+");

   }
   /**
    * Valida el tipo de una embarcación.
    *
    ...
    */
   public static boolean validarTipoNave(String tipo) {

       return noEsVacio(tipo)
               && soloLetras(tipo)
               && longitudMinima(tipo, 3);
